...
func(bf *BloomFilter) hashString(hashSeed int8, s string) (hashValue int64) {
...
func(bf *BloomFilter) SetString(s string) error{
	if len(s) > 20 {
		return errors.New("exceed max string length")
	}

	for _, seed := bf.hashSeeds {
		value := hash(seed, s)
...
func(bf *BloomFilter) CheckString(s string) (exist bool) {
	exist = true
